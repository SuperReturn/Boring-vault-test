...
contract TellerWithMultiAssetSupportTest is Test, MainnetAddresses {
    ...
    function setUp() external {
        ...
        rolesAuthority.setRoleCapability(
            ADMIN_ROLE, address(teller), TellerWithMultiAssetSupport.setStakingVaultTeller.selector, true
        );
        rolesAuthority.setRoleCapability(
        ...
        rolesAuthority.setPublicCapability(address(teller), TellerWithMultiAssetSupport.depositAndStake.selector, true);
    ...
    //     users[0] = user;
    //     atomicSolverV4.redeemSolve(atomicQueue, boringVault, WETH, users, 0, type(uint256).max, teller, req);
    //     vm.stopPrank();
    // }
    ...
    function testDepositAndStake() external {
        // Setup user
        vm.startPrank(user);
        MockUSDC(address(USDC)).mint(user, 1_000_000_000e6);
        USDC.approve(address(boringVault), 1_000_000_000e6);
        boringVault.approve(address(teller), 1_000_000_000e6);
        teller.depositAndStake(USDC, 1_000_000_000e6, 0);
