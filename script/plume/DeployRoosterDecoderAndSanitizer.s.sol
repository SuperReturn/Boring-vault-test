...
import {RoosterDecoderAndSanitizer} from "src/base/DecodersAndSanitizers/Protocols/RoosterDecoderAndSanitizer.sol";
...
contract DeployRoosterDecoderAndSanitizerScript is Script, ContractNames, PlumeAddresses, MerkleTreeHelper {
    ...
    function run() external {
        ...
        creationCode = type(RoosterDecoderAndSanitizer).creationCode;
        constructorArgs = abi.encode(
            previoussuperUSD
        );
        deployer.deployContract(UsdaiRoosterDecoderAndSanitizerName, creationCode, constructorArgs, 0);
